import { ApiClients, ApiReceipts } from "@/shared/api";
...
export default function useModule() {
  ...
  const getClients = async () => {
    try {
      const data: any = await ApiClients.getClients(1, 100);
  ...
  const getReceipts = async () => {
    try {
      const data: any = await ApiReceipts.getReceipts(1, 100);
      receipts.value = data.data.map((item) => ({
        id: item.id,
        name: item.name,
      }));
    } catch (e: any) {
  ...
  onMounted(async () => {
    try {
      ...
      await Promise.allSettled([getClients(), getReceipts()]);
    } catch (e: any) {
      console.log("Error: ", e);
    } finally {
