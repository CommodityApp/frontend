...
const deleteSingleProducer = (id, name) => {
  window.confirm(`Вы действительно хотите удалить ${name} клиента?`)
    ? props.deleteProducer(id)
    : null;
};
const editSingleProducer = (id) => {
  props.editProducer(id);
};
</script>
<template>
