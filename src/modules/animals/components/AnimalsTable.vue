...
const deleteSingleAnimalType = (id, name) => {
  window.confirm(`Вы действительно хотите удалить ${name} тип животного?`)
    ? props.deleteAnimalTypes(id)
    : null;
};
        ...
        <table class="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-6 py-4">
                <span v-for="(type, index) in animals.children" :key="index">
                  {{ type.name }}&nbsp;-&nbsp;
                </span>
              </td>
